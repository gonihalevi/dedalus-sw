...
# dt(um) + g*km*h - 2*Om*i*C*um + nu*km.kp.km.kp.um = - (u.grad u)_m
# dt(up) + g*kp*h + 2*Om*i*C*up + nu*km.kp.km.kp.up = - (u.grad u)_p
...
def shallow_water(S,m,params):
    ...
    g,H,Om,a,nu  = params[0],params[1],params[2],params[3],params[4]

    # (-,-)
    L00 = -2*Om*1j*S.op('C',m,-1) + nu*(S.op('k-',m,0).dot(
                                        S.op('k+',m,-1).dot(
                                        S.op('k-',m,0).dot(
                                        S.op('k+',m,-1)))))
    ...
    # (+,+)
    L11 = 2*Om*1j*S.op('C',m,+1)  + nu*(S.op('k+',m,0).dot(
                                        S.op('k-',m,+1).dot(
                                        S.op('k+',m,0).dot(
                                        S.op('k-',m,+1)))))
